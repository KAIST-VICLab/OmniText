"""Run OmniText training-free text editing (and style-based editing with --ref ref2) over OmniText-Bench.

Extracted from the original OmniText_Editing_Ref1 notebook (kept verbatim under
notebooks/legacy/). Hyperparameters come from a YAML config (configs/editing.yaml);
...
Example:
    python scripts/run_editing.py --config configs/editing.yaml \
        --dataset-root OmniText-Bench --output-dir experiments/editing --ref ref1
"""
import argparse
import os
...
def parse_args():
    ...
    p.add_argument("--config", default="configs/editing.yaml", help="Path to the YAML config.")
    p.add_argument("--dataset-root", default="OmniText-Bench", help="Path to the OmniText-Bench directory.")
    p.add_argument("--output-dir", default="experiments/editing", help="Where to write per-image outputs.")
...
def main():
    ...
    for key in tqdm(labels):
        with torch.no_grad():
            anns = labels[key]['application']['editing']
            out_subdir = os.path.join(out_dir, key)
            Path(out_subdir).mkdir(parents=True, exist_ok=True)
        
            ref_image_path = str(omnidata.ref_image_path(args.dataset_root, "editing", ref, key))
            ...
            for idx_ann in range(1):
                source_text = anns[ref]['text']['target']
                target_text = anns[ref]['text']['target']
                if True:
