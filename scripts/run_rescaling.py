"""Run OmniText training-free text rescaling over OmniText-Bench.

Extracted from the original OmniText_Rescaling notebook (kept verbatim under
notebooks/legacy/). Hyperparameters come from a YAML config (configs/rescaling.yaml);
...
Example:
    python scripts/run_rescaling.py --config configs/rescaling.yaml \
        --dataset-root OmniText-Bench --output-dir experiments/rescaling
"""
import argparse
import os
...
def parse_args():
    ...
    p.add_argument("--config", default="configs/rescaling.yaml", help="Path to the YAML config.")
    p.add_argument("--dataset-root", default="OmniText-Bench", help="Path to the OmniText-Bench directory.")
    p.add_argument("--output-dir", default="experiments/rescaling", help="Where to write per-image outputs.")
...
def main():
    ...
    for key in tqdm(labels):
        with torch.no_grad():
            anns = labels[key]['application']['rescaling']
