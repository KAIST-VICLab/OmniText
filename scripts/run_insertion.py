"""Run OmniText training-free text insertion (and style-based insertion with --ref ref2) over OmniText-Bench.

Extracted from the original OmniText_Insertion_Ref1 notebook (kept verbatim under
notebooks/legacy/). Hyperparameters come from a YAML config (configs/insertion.yaml);
...
Note: insertion operates on the original input image; the
``--removal-output-dir`` argument is accepted for interface parity but unused.
...
def parse_args():
    ...
    p.add_argument("--config", default="configs/insertion.yaml", help="Path to the YAML config.")
    p.add_argument("--dataset-root", default="OmniText-Bench", help="Path to the OmniText-Bench directory.")
    p.add_argument("--output-dir", default="experiments/insertion", help="Where to write per-image outputs.")
    p.add_argument("--removal-output-dir", default="experiments/removal",
                   help="Directory of removal.png outputs (run_removal.py first).")
    p.add_argument("--ref", choices=["ref1", "ref2"], default="ref1",
                   help="Reference style: ref1 (plain) or ref2 (style-based).")
...
def main():
    ...
    for key in tqdm(labels):
        with torch.no_grad():
            anns = labels[key]['application']['insertion']
            out_subdir = os.path.join(out_dir, key)
            Path(out_subdir).mkdir(parents=True, exist_ok=True)
        
            ref_image_path = str(omnidata.ref_image_path(args.dataset_root, "insertion", ref, key))
            ref_image = Image.open(ref_image_path).convert("RGB")
    
            inp_image_path = str(omnidata.input_image_path(args.dataset_root, key))
            ...
            for idx_ann in range(1):
                source_text = anns[ref]['text']
                target_text = anns[ref]['text']
                if True:
                    ...
                    ref_mask_polygon = anns[ref]['ref_polygon']
                    target_mask_polygon = anns[ref]['target_polygon']
            ...
            adaptive_scale_con = cfg.get("adaptive_scale_con") or (2 if ref == "ref2" else 5) # 2 can also be used, feel free to change this
